...
No_of_elements_vec = [4,16,64,256,1024,4096];
space_order_of_Basis = 3;

NO_time_step_vec = [4]; %% 时间步和阶数可以是向量，看loop
time_order_of_Basis_vec = [2];

Basis_Type  = 'Q';     % Q,Se

Static_Condensation = 'on'; % on, off
for cc = 1:length(No_of_elements_vec)
        No_of_elements = No_of_elements_vec(cc);
...
coef1=coef_vector(:,k-1,t);
    ...
    [L_2_L2_part,L_2_grad_part,L_2_H1_part] = Err_timeL2_norm(Space_Node,Time_Node,BDbox,...
                                     coef,S_Po,T_Po,FEM_index,@(x)u(x) ,@(x)grad_u(x) ,@(x)zeta(x));
...
energynorm_err_vector(k) = sqrt( energynorm_err_vector(k)^2+H1_H1_err_vector(k)^2 ...
