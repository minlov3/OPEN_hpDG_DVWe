function local = CR_localmass(Space_Node,Time_Node,BDbox,S_Po,T_Po,FEM_index, b, c)
...
weights = kron(Space_weights,Time_weights);
 ...
 P_Qpoints = [quad_x,quad_y,quad_t];
    ...
    b_val = b(P_Qpoints);    c_val = c(P_Qpoints) ;

first = zeros(dim_elem,dim_elem); second = zeros(dim_elem,dim_elem);
    
for i = 1:dim_elem
   
    for j=1:dim_elem
    
        % first term {cuv} is symetric
         
        t = c_val.*FEM2D_DG_basis(P_Qpoints,BDbox,m,h,FEM_index(i,:))...
                 .*FEM2D_DG_basis(P_Qpoints,BDbox,m,h,FEM_index(j,:));
        
        first(j,i) = dot((t),weights);
             
        %  second term b \cdot grad(u)v is 
        %  skew symmetric
             
       s = sum(b_val.* vt_grad_FEM2D_DG_basis(P_Qpoints,BDbox,m,h,FEM_index(i,:)),2).*vt_FEM2D_DG_basis(P_Qpoints,BDbox,m,h,FEM_index(j,:));
              
        
        second(j,i) = dot((s),weights);
