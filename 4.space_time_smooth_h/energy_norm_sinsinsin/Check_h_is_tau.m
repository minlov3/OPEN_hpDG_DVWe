...
NO_elem_vec = [4,16,64,256,1024,4096]; 
NO_time_step_vec = [2,4,8,16,32,64];
...
for i=1 :length(NO_time_step_vec)    
     load(['Error of ' num2str(NO_elem_vec(i)) ' rectangle space Elements combine ' num2str(NO_time_step_vec(i)) ' time elements with space basis '...
            Basis_Type num2str(T_Polydegree_vec(k)) ' and time basis ' num2str(T_Polydegree_vec(k)) ' basis.mat'])
...
switch  Norm_type
    ...
    case 'H1_grad'
     ...
     case 'H1_H1'    
        ...
        title('$\|e\|_{\mathrm{E}}$ norm error for smooth solution under $h$-refinement','FontSize',18,'interpreter','LaTex')
     case 'W_1_inf_L2'
     ...
     case 'Final_t_L2_grad'
        ...
        title('$||\nabla e(T)||_{L^2(\Omega))}$ for smooth solution under $h$-refinement','FontSize',18,'interpreter','LaTex')        
     case 'Final_t_L2_H1'    
        ylabel('$||\nabla e(T)||_{L^2(\Omega))}$','FontSize',18,'interpreter','LaTex');
        title('$||\nabla e(T)||_{L^2(\Omega))}$ for smooth solution under $h$-refinement','FontSize',18,'interpreter','LaTex')      
     case 'Final_t_H1_L2'
        ylabel('$||\dot{e}(T)||_{L^2(\Omega))}$','FontSize',18,'interpreter','LaTex');
        title('$||\dot{e}(T)||_{L^2(\Omega))}$ for smooth solution under $h$-refinement','FontSize',18,'interpreter','LaTex');
