...
load([num2str(eta(ze)) ' Energy of ' num2str(NO_elem ) ' rectangle space Elements combine ' num2str(NO_time_step) ' time elements with space basis '...
...
format shortE
legend([ '\eta=' num2str(eta(1)) '      with slope  ' num2str(-slope_Poly(1))  ],...
       [ '\eta=' num2str(eta(2)) ' with slope ' num2str(-slope_Poly(2))  ],...
       [ '\eta=' num2str(eta(3)) '   with slope ' num2str(-slope_Poly(3))  ],...
       [ '\eta=' num2str(eta(4)) '   with slope ' num2str(-slope_Poly(4))  ],...
       [ '\eta=' num2str(eta(5)) '      with slope ' num2str(-slope_Poly(5))  ],...
       'Location','SouthWest')
...
title(['$||{u}^{\prime}_{\tau}(t_n^-)||$  under the coefficient $\gamma=0$, $q=$', num2str(T_Polydegree_vector(1)), ', $N_T=50$'],'FontSize',18,'interpreter','LaTex');
