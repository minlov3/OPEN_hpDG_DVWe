...
NO_time_step_vec = [50]; %% 时间步和阶数可以是向量，看loop
time_order_of_Basis_vec = [6];
...
time_FEM_index1 = time_FEM_index;
for delete = 1:length(time_FEM_index)
if time_FEM_index(delete)==0
   time_FEM_index1(delete)=[];
end
end
time_FEM_index = time_FEM_index1;


FEM_index = [kron(Space_FEM_index,ones(length(time_FEM_index),1)),...
...
i = zeros(length(time_FEM_index),size(Diribd_index,1) );  s = zeros(length(time_FEM_index),size(Diribd_index,1) ); 
...
parfor t =1:size(Diribd_index,1)
   ...
   Diri_Nodal = DiriBC_projecton_nodal(Space_Node,Time_Node,S_Po,T_Po, S_Polydegree,time_FEM_index,@(x)g_D(x)); 
    ...
    [Final_t_L2_L2_part, Final_t_L2_grad_part,Final_t_L2_H1_part,...
     ...
     Final_t_H1_H1_err(t)  = Final_t_H1_H1_part; 
        ...
        = Err_Jump_time_norm(Space_Node,Time_Node,Time_Node1,BDbox,BDbox1,...
                                     coef,coef1, S_Po,T_Po , FEM_index, @(x)zeta(x));
...
dg_ene_vector(k) = dg_ene_vector(k) ;
DG_ene_vector(k) = sqrt(Final_t_L2_grad_ene_vector(k)+ Final_t_H1_L2_ene_vector(k));
...
meeta=gamma(x0);
eeta=meeta(9);
savefile = [num2str(eeta) ' Energy of ' num2str(NT) ' rectangle space Elements combine ' num2str(NO_time_step) ' time elements with space basis '...
...
meeta=gamma(x0);
eeta=meeta(9);
savefile = [num2str(eeta) ' Error of ' num2str(NT) ' rectangle space Elements combine ' num2str(NO_time_step) ' time elements with space basis '...
            Basis_Type num2str(S_Polydegree) ' and time basis ' num2str(T_Polydegree) ' basis.mat'];
