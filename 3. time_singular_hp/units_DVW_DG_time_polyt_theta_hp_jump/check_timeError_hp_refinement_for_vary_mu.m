...
Time_Step_Vector = [1,2,3,4,5,6,7,8];

gf = 0.25;     % temporal mesh grading factor of 
mu_vec = [1,1.5,1.75,2,2.25,2.5];  % polynomial degree increasing factor
...
loglog(dof_P.^a,err_P(:,1),['-.s' num2str(color{OA})],'LineWidth',1.5,'MarkerSize',10);
     case 'PQ'
loglog(dof_P.^a,err_P(:,1),['-b' num2str(color{OA})],'LineWidth',1.5,'MarkerSize',10);
     case 'Q'
semilogy(dof_P.^a,err_P(:,1),['-*' num2str(color{OA})],'LineWidth',1.5,'MarkerSize',10);
end
