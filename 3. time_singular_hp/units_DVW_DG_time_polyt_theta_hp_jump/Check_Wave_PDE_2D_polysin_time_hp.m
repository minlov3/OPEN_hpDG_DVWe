...
Time_Step_Vec = [1,2,3,4,5,6,7,8,9];

gf_vec = [0.15] ;   % temporal mesh grading factor of 
mu_vec = [1,1.25,1.5,1.75,2,2.25,2.5]; % polynomial degree increasing factor
...
for OB = 1: length(mu_vec) 
    
gf = gf_vec(OA);
mu = mu_vec(OB);
   
for  Time_Step_Index = 1: size(Time_Step_Vec,2)

    NO_time_step = Time_Step_Vec(Time_Step_Index);
...
S = gf.^(NO_time_step-1:-1:0).*time_end;
t_grids = [0,S];
...
T_Polydegree_vec = [1, floor(mu.*(2:NO_time_step)) ]' ;  % polynomial degree distribution  
...
disp([num2str(No_of_elements) ' space elements with ' Basis_Type num2str(S_Polydegree) ' space basis, combining ' ...
      num2str(Time_Step_Vec(Time_Step_Index) ) ' time steps with grading factor sigma=' num2str(gf)...
      ' and polynomial degree increasing factor mu=' num2str(mu) ])
...
dim_FEM_vec = NaN(NO_time_step,1);
dim_time_vec = NaN(NO_time_step,1);
coef_vector = cell(NO_time_step,NO_space_element);%%%%%%%%%%%%%%%%%%%此时需要一个cell来存储coef
FEM_index_vector = cell(NO_time_step,1);
for k= 1 : NO_time_step

    
T_Polydegree=T_Polydegree_vec(k);   
T_Po = T_Polydegree*2+1;         % quadrature order 
    
    
%% Q basis for 2D space and 1D time

Space_FEM_index = FEM_Q_Basis_index2D(S_Polydegree);
...
%NO_ModalBasis = (S_SPolydegree-1)^2.*NT; %%上面的泛用性更强，但这里其实只用Q元从而一样的
...
dim_FEM_vec(k) = step_dim_FEM; 
dim_time_vec(k) = No_timeBasis;
...
for t =1:NO_space_element
   ...
   if S_Polydegree == 1
    ...
    coef_vector{k,t}=global_ind;  
       
   end 

   
end
%%%%%%%%%%%%%%%%%%%%%%%%%我们在上面只是生成了为了生成数组，我们实际需要的是U(global)，在后面强制修改就好
...
% k>=2 we need to recall the solution of the previous time steps
   ...
   % get the space 2D elements and build up to the local 3D prism elements 
    ...
    global_nodal_index = get_space_tensor_time_index(elem,No_timeBasis_previous); 
   
   
   if S_Polydegree_previous == 1
       ...
       space_EdgeBasis_ind = get_EdgeBasisindex(local_edge,TotalEdge,S_Polydegree_previous);  
       % changle the index set from space to space-time
       
       global_EdgeBasis_ind = get_space_tensor_time_index(space_EdgeBasis_ind,No_timeBasis_previous); 
       
       % find all the modal basis 
       
       NO_local_modalbasis = dim_elem_previous - size(global_nodal_index,1) - size(global_EdgeBasis_ind,1);
       ...
       global_ind = [global_nodal_index ;...
              (global_EdgeBasis_ind + NO_NodalBasis_previous*No_timeBasis_previous);...
              (global_modal_index + (NO_NodalBasis_previous+NO_EdgeBasis_previous)*No_timeBasis_previous)];
    ...
    inflowface = CR_vect_inflowface_previous_step(Space_Node,Time_Node, BDbox, previous_BDbox, previous_coef,  norvec ,S_Po,T_Po ,FEM_index, FEM_index_previous,@(x)a(x),@(x)zeta(x),@(x)b(x));    
...
U(int_index) = StaticCondensation_FEM(B(int_index,int_index),L(int_index),NT,(S_Polydegree-1)^2*No_timeBasis);  

end


%% passing all the information to next time step!

U_previous = U; 
FEM_index_previous = FEM_index;

No_timeBasis_previous = No_timeBasis;

NO_NodalBasis_previous = NO_NodalBasis;

NO_EdgeBasis_previous = NO_EdgeBasis;

NO_ModalBasis_previous = NO_ModalBasis;


S_Polydegree_previous = S_Polydegree;

dim_elem_previous = dim_elem;
...
if k==1
    coef1=coef;
    FEM_index1=FEM_index;
else
coef1=coef_vector{k-1,t};
FEM_index1=FEM_index_vector{k-1};
end    
    ...
    [Final_t_L2_L2_part, Final_t_L2_grad_part, Final_t_L2_H1_part, ...
     ...
     = Err_final_time_norm(Space_Node,Time_Node,BDbox,coef, S_Po,T_Po , FEM_index, @(x)u(x), @(x)grad_u(x), @(x)grad_ut(x) ,@(x)a(x));
     ...
     Final_t_H1_H1_err(t)  = Final_t_H1_H1_part; 
        ...
        = Err_Jump_time_norm(Space_Node,Time_Node,Time_Node1,BDbox,BDbox1,...
                                     coef,coef1, S_Po,T_Po , FEM_index,FEM_index1, @(x)a(x));
...
dim_FEM = sum(dim_FEM_vec);
S_dim = step_space_dim_FEM;
T_dim = sum(dim_time_vec);
...
savefile = ['Error of ' num2str(NT) ' rectangle space Elements with ' Basis_Type num2str(S_Polydegree) ' space basis, combining '...
             num2str( Time_Step_Vec(Time_Step_Index) ) ' time step with sigma = ' num2str(gf) ' and  mu = ' num2str(mu) '.mat'];
...
save(savefile, 'L2_L2_err','L2_grad_err','L2_H1_err',...
               ...
               'Final_t_L2_L2_err','Final_t_L2_grad_err','Final_t_L2_L2_err',...
               'Final_t_H1_L2_err','Final_t_H1_grad_err','Final_t_H1_H1_err',...
               'Time_jump_u_err','Time_jump_gradu_err','Time_jump_dotu_err',...
               'Energynorm_err',...
               'dim_FEM','S_dim','T_dim','S_Polydegree','Time_Step_Vec','T_Polydegree_vec','-v7.3');
