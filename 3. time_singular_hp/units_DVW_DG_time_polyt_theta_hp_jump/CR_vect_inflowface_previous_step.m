function z_vect = CR_vect_inflowface_previous_step(Space_Node,Time_Node,  BDbox, previous_BDbox, previous_coef, n ,S_Po,T_Po ,FEM_index,FEM_index_previous, a,zeta, b)
...
dim_elem_previous = size(FEM_index_previous,1);
...
previous_m = 0.5.*sum(previous_BDbox);
   ...
   P1 = zeros(size(P_Qpoints,1) ,dim_elem_previous);
   P2 = zeros(size(P_Qpoints,1) ,dim_elem_previous);
   P3 = zeros(size(P_Qpoints,1) ,dim_elem_previous); 
    for i =1:dim_elem_previous
        
        P1(:,i)= vx_FEM2D_DG_basis(P_Qpoints,previous_BDbox,previous_m,previous_h,FEM_index_previous(i,:));
        P2(:,i)= vy_FEM2D_DG_basis(P_Qpoints,previous_BDbox,previous_m,previous_h,FEM_index_previous(i,:));
        P3(:,i)= vt_FEM2D_DG_basis(P_Qpoints,previous_BDbox,previous_m,previous_h,FEM_index_previous(i,:));
    end
