function [Time_jump_u_part, Time_jump_gradu_part, Time_jump_dotu_part]...
    = Err_Jump_time_norm(Space_Node,Time_Node,Time_Node1,BDbox,BDbox1 , coef,coef1 ,S_Po,T_Po  ,FEM_index,FEM_index1,a)
...
dim_elem1 =size(FEM_index1,1); % number of basis for each element.
...
%%Calculating the DG norm error based on the old way
 ...
 % data for quadrature
    ...
    % construct the matrix for all the local basis function
     ...
     P1 = zeros(size(P_Qpoints,1) ,dim_elem1);
    Px = zeros(size(P_Qpoints,1) ,dim_elem);
    P1x = zeros(size(P_Qpoints,1) ,dim_elem1);
    Py = zeros(size(P_Qpoints,1) ,dim_elem);
     P1y = zeros(size(P_Qpoints,1) ,dim_elem1);
    Pz = zeros(size(P_Qpoints,1) ,dim_elem);
     P1z = zeros(size(P_Qpoints,1) ,dim_elem1);
    ...
    for i =1:dim_elem
        ...
        P1(:,i)= FEM2D_DG_basis(P_Qpoints,BDbox1,m1,h1,FEM_index1(i,:));
        t1 = grad_FEM2D_DG_basis(P_Qpoints,BDbox1,m1,h1,FEM_index1(i,:));
        v1 = vt_grad_FEM2D_DG_basis(P_Qpoints,BDbox1,m1,h1,FEM_index1(i,:));      
