function [W_1_inf_L2_part,W_1_inf_grad_part,W_1_inf_H1_part] = Energy_timeW_1_inf_norm(Space_Node,Time_Node,BDbox , coef ,S_Po, T_Po,FEM_index,u,grad_u,grad_ut,a)
...
W_1_inf_L2_part = NaN(T_Po,1);
W_1_inf_grad_part = NaN(T_Po,1);
W_1_inf_H1_part = NaN(T_Po,1);
...
%%Calculating the DG norm error based on the old way
 ...
 % data for quadrature
    ...
    grad_ut_DG = [Qx*coef , Qy*coef, Qz*coef];
     ...
     t1 = ( 0 - grad_u_DG(:,3) ).^2;          
     W_1_inf_L2_part(k) = dot((t1),weights);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
     % L2 norm of the H_1 semi-norm    
     grad1 = 0 - grad_ut_DG;    grad2 = 0 - grad_ut_DG;
     ...
     t3 = sum((grad).*a_val,2);   
     W_1_inf_grad_part(k) = dot((t3),weights);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     
     % H1=L2+grad
     
     W_1_inf_H1_part(k)  =   W_1_inf_grad_part(k) + W_1_inf_L2_part(k);%%%%%%%%%%
